 ...
 * hex field + preset and recent swatches. 26.x extractor rendering.
 */
public class ColorPickerScreen extends Screen {
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean bl) {
        // Left click is button 0 on GLFW versions (<=26.2) and button 1 on
        // SDL versions (26.3+) - accept both, like vanilla's click handling.
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        graphics.centeredText(this.font, Component.literal("drag the square / hue bar, type hex, or click a swatch"), this.width / 2, 22, 0xAAAAAA);
        ...
        graphics.outline(sqX, sqY, sqW, sqH, 0xFF000000);
        // crosshair at current sat/bright
        int cxm = sqX + (int) (sqW * sat);
        int cym = sqY + (int) (sqH * (1.0f - bright));
        graphics.outline(cxm - 3, cym - 3, 7, 7, 0xFF000000);
        graphics.outline(cxm - 2, cym - 2, 5, 5, 0xFFFFFFFF);
        ...
        graphics.outline(hueX, sqY, hueW, sqH, 0xFF000000);
        ...
        graphics.outline(rightX, previewY, rightW, 28, 0xFF000000);
        graphics.text(this.font, Component.literal("Hex:"), rightX, hexY - 10, 0xAAAAAA);
        graphics.text(this.font, Component.literal("Presets:"), rightX, presetY, 0xAAAAAA);
        for (int i = 0; i < PRESETS.length; i++) {
            int x = rightX + (i % 6) * 17;
            int y = presetY + 12 + (i / 6) * 17;
            graphics.fill(x, y, x + 15, y + 15, PRESETS[i] | 0xFF000000);
            graphics.outline(x, y, 15, 15, 0xFF000000);
        }
        graphics.text(this.font, Component.literal("Recent:"), rightX, recentY, 0xAAAAAA);
        for (int i = 0; i < InvisSeeConfig.recentColors.size() && i < 6; i++) {
            int x = rightX + i * 17;
            graphics.fill(x, recentY + 12, x + 15, recentY + 27, InvisSeeConfig.recentColors.get(i));
            graphics.outline(x, recentY + 12, 15, 15, 0xFF000000);
        }
